...
# Analisa Foreign Flow + KSEI bulanan (dari ksei_month)
...
st.caption("Harga + Foreign Flow + Partisipasi Asing/Ritel (KSEI bulanan dari `ksei_month`).")
...
with engine.connect() as con:
    if USE_EOD_TABLE:
        syms = pd.read_sql("SELECT DISTINCT base_symbol FROM eod WHERE is_foreign_flow=0 ORDER BY base_symbol", con)["base_symbol"].tolist()
    else:
        syms = pd.read_sql("""
...
if USE_KSEI and df["foreign_pct"].notna().any():
    ...
    df["Ri_pct"] = pd.to_numeric(df["retail_pct"].where(df["retail_pct"].notna(), 100 - df["Pa_pct"]), errors="coerce").clip(0,100)
else:
...
if USE_KSEI:
    ...
    if not kdf.empty:
        ...
        # agregasi bulan (pastikan kolom Month tidak hilang)
        agg = (kdf.sort_values("trade_date")
                 .groupby("Month", as_index=False)
                 .agg({"total_volume":"sum", "foreign_pct":"mean"}))

        # hitung vol foreign/local by Month dengan groupby → reset_index (aman)
        vol_foreign = (kdf
                       .assign(foreign_frac = pd.to_numeric(kdf["foreign_pct"], errors="coerce")/100.0)
                       .groupby("Month", as_index=False)
                       .agg(vol_foreign = ("total_volume", lambda s: float((s * kdf.loc[s.index, "foreign_pct"].fillna(0)/100.0).sum())),
                            vol_local   = ("total_volume", lambda s: float((s * (1.0 - kdf.loc[s.index, "foreign_pct"].fillna(0)/100.0)).sum()))))
        ...
        if chart_type == "Line":
            sub.add_trace(go.Scatter(x=vol_foreign["Month"], y=vol_foreign["vol_foreign"],
                                     name="Foreign (est.)", mode="lines+markers"), row=1, col=1)
            sub.add_trace(go.Scatter(x=vol_foreign["Month"], y=vol_foreign["vol_local"],
                                     name="Lokal/Ritel (est.)", mode="lines+markers"), row=1, col=1)
        else:
            sub.add_trace(go.Bar(x=vol_foreign["Month"], y=vol_foreign["vol_foreign"], name="Foreign (est.)"), row=1, col=1)
            sub.add_trace(go.Bar(x=vol_foreign["Month"], y=vol_foreign["vol_local"],  name="Lokal/Ritel (est.)"), row=1, col=1)
