...
def get_latest_trade_date(table_name: str) -> date:
    ...
    try:
        with engine.connect() as con:
            ...
            if isinstance(d, datetime):
                d = d.date()
            return d if d else date(1990, 1, 1)
    except Exception:
        return date(1990, 1, 1)
...
# ────────────────────────────────────────────────────────────────────────────────
@st.cache_data(ttl=1800)  # kurangi risiko 429 & hemat request
...
def fetch_idr_from_sheets(sheet_id: str, gid: str = "0") -> pd.DataFrame:
    ...
    try:
        ...
        df["IDR_USD"] = df["IDR_USD"].astype(str).str.replace(r"[^\d\.\-]", "", regex=True)
...
try:
    with engine.connect() as con:
        ...
        """)
        min_date_db = con.execute(q).scalar() or date(1990, 1, 1)
        if isinstance(min_date_db, datetime):
            min_date_db = min_date_db.date()
except Exception:
    min_date_db = date(1990, 1, 1)

st.session_state.min_date_db = min_date_db
DEFAULT_START_DATE_MANUAL = date(2003, 12, 1)
default_start_value = max(DEFAULT_START_DATE_MANUAL, st.session_state.min_date_db)

def _normalize_date(val, fallback: date) -> date:
    """Pastikan return tipe date dan bukan None."""
...
# Inisialisasi nilai widget key jika belum valid
if st.session_state.get("filter_start") in (None, ""):
    st.session_state["filter_start"] = _normalize_date(st.session_state.start_date_filter, default_start_value)
if st.session_state.get("filter_end") in (None, ""):
    st.session_state["filter_end"] = _normalize_date(st.session_state.end_date_filter, end_date_today)

# Nilai untuk pertama render
prefill_start = _normalize_date(st.session_state.get("filter_start"), default_start_value)
prefill_end   = _normalize_date(st.session_state.get("filter_end"), end_date_today)

selected_start_date = st.sidebar.date_input(
    "Tanggal Mulai",
    value=prefill_start,
    min_value=st.session_state.min_date_db,
    max_value=end_date_today,
    key="filter_start",
)
selected_end_date = st.sidebar.date_input(
    "Tanggal Akhir",
    value=prefill_end,
    min_value=_normalize_date(selected_start_date, st.session_state.min_date_db),
    max_value=end_date_today,
    key="filter_end",
)

# Normalize lagi dan handle swap jika start > end
selected_start_date = _normalize_date(selected_start_date, default_start_value)
selected_end_date   = _normalize_date(selected_end_date, end_date_today)
if selected_start_date > selected_end_date:
    selected_start_date, selected_end_date = selected_end_date, selected_start_date
    st.info("ℹ️ Tanggal mulai melebihi tanggal akhir, otomatis ditukar.")
